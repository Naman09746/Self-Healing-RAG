...
def _split_text_recursively(
    ...
    # If separator was a header prefix like "\n## ", splitting directly removes "\n## ".
    # Handle header separators cleanly by re-attaching prefix or splitting by newline before header.
    if separator.startswith("\n#"):
        ...
        for idx, part in enumerate(raw_parts):
            if idx == 0:
                ...
                splits.append(separator.lstrip("\n") + part)
    else:
...
def _split_table_smartly(table_block: str, chunk_size: int) -> List[str]:
    """Bisect oversized markdown table row-by-row while preserving header row on slices."""
    ...
    slices: List[str] = []
    cur_rows: List[str] = []
    header_overhead = len(header_block) + 2

    for row in data_rows:
        row_len = len(row) + 1
        if cur_rows and (header_overhead + sum(len(r) + 1 for r in cur_rows) + row_len > chunk_size):
            slice_content = header_block + "\n" + "\n".join(cur_rows)
    ...
    return slices or [table_block]


def _split_markdown_aware(text: str, chunk_size: int, chunk_overlap: int) -> List[str]:
    """Split markdown text while preserving code blocks, tables, and section hierarchies."""
    if not text:
        return []

    # Unicode normalization (NFKC) for uniform character representation
    text = unicodedata.normalize("NFKC", text.replace("\r\n", "\n"))

    # If document has no markdown code blocks or tables, use recursive hierarchy splitter directly
    has_code = "```" in text
    table_pattern = re.compile(r'((?:^[ \t]*\|[^\n]+\|[ \t]*\n)+)', re.MULTILINE)
    has_tables = bool(table_pattern.search(text))

    if not has_code and not has_tables:
        return _split_text_recursively(text, chunk_size, chunk_overlap)

    # Tokenize text into atomic units (code blocks, table blocks, and prose segments)
    ...
    for part in parts:
        ...
        if part.startswith("```") and part.endswith("```") and len(part) >= 6:
            ...
            # Check for tables within non-code prose
            table_subparts = table_pattern.split(part)
            for subpart in table_subparts:
                if not subpart:
                    continue
                trimmed = subpart.strip()
                if not trimmed:
                    continue
                # Is it a table block?
                if trimmed.startswith("|") and trimmed.endswith("|") and "\n|" in trimmed:
                    if len(trimmed) <= chunk_size:
                        atomic_units.append(trimmed)
                    else:
                        atomic_units.extend(_split_table_smartly(trimmed, chunk_size))
                else:
                    if len(trimmed) <= chunk_size:
                        atomic_units.append(trimmed)
                    else:
